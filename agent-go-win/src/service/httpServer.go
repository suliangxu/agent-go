...
func ProcessRequest(w http.ResponseWriter, FormMap sync.Map) (string, string) {
	var res string
	...
	if !ok{
		...
		fmt.Fprintln(w, res)
		Info("get process " + res)
		return "",""
	}

	filename, ok := FormMap.Load("AgentVersion")
	if !ok{
		res = "no AgentVersion parameter"
		fmt.Fprintln(w, res)
		Info("get process " + res)
		return "",""
	}
	...
	filename = "agent-go-win-" + filename.(string) + ".zip"
...
func updateProject(w http.ResponseWriter, r *http.Request)  {
	...
	url, filename := ProcessRequest(w, FormMap)

	if url!=""{
		Info("start update project from" + url)
		go projectUpdate(url, filename)
	}
}
...
func getAgentInfo(w http.ResponseWriter, r *http.Request)  {
	...
	Info(r.RemoteAddr + " connect the server getAgentInfo")
...
func getProcessInfo(w http.ResponseWriter, r *http.Request)  {
	Info(r.RemoteAddr + " connect the server getProcessInfo")
...
func getUninstallSignal(w http.ResponseWriter, r *http.Request)  {
	...
	configure.UninstallProjectSignal = true

	fmt.Fprintln(w, "true")

	go RemoveOldProjectAndKill()
}
...
func HttpServer()  {
	...
	Info("建立监听，路径为 http://" + port + "/updateScript")
	Info("建立监听，路径为 http://" + port + "/updateProject")
